...
for i=1:nsteps
    fa = fa + 0.5 * ob_ix(i)*(innov(na+1:end,i)-H(na+1:end,na+1:end)*dz(na+1:end,i+1))'...
...
g_hat(na+1:na+no,nsteps+1) = g_hat(na+1:na+no,nsteps+1) - ob_ix(i)*invR*...
...
%   One step of adjoint model  
    ...
    g_hat(na+1:end,i) = g_hat_model(na+1:end,1) - ob_ix(i-1).*invR*...
