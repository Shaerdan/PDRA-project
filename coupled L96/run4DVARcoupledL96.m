...
var_atmos_bg = 1*1e-2; var_ocean_bg = 1*1e-2;
var_ob = [1*1e-2, 1*1e-2];
% loop controls:
        ...
        for i_smooth_iteration=1:s5_iterations
            for i_cycles = 1:n_cycles_per_smoother
                ...
                if assim_scheme == 4
                    ...
                    error_norm_analysis(indx_str:indx_end+1) = vecnorm(za_chk - z);
                    error_norm_bg(indx_str:indx_end+1) = vecnorm(zb_f_chk - z);
                else
...
%                     legend show
            ...
            if assim_scheme == 5
                ...
                xlabel('Assimilation Steps')
                legend show
                error_norm_postsmoother = vecnorm(za2_f(na+1:end,:) - z(na+1:end,:));
                error_norm_presmoother = vecnorm(za_chk(na+1:end,:) - z(na+1:end,:));
...
if save_all_figures == 1
    ...
    dirname = 'C:\07022023\results\1319\';
    mkdir(dirname);
