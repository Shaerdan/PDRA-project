function [f,g] = calcfg_atmos_l96c(dX0_a,z_b,innov,z_lin,H_atmos,invB,invR,nsteps,h,na,no,Fx,Fy,alph,gamma,ob_ix)
...
for i=1:nsteps
    fa = fa + 0.5 * ob_ix(i)*(innov(:,i)-H_atmos*dz(1:na,i+1))'...
        *invR*(innov(:,i)-H_atmos*dz(1:na,i+1));
end
f = fa;

%% observation gradient at the last step:
g_hat = zeros(na+no,nsteps+1);
g_hat(1:na,nsteps+1) = g_hat(1:na,nsteps+1) - ob_ix(nsteps)*H_atmos'*invR*...
    (innov(:,nsteps)-H_atmos*dz(1:na,nsteps+1));
% adjoint interation:
for i=nsteps:-1:2
    dfdz_adj = dfdz(:,:,i);
    dfdYi_adj = dfdYi(:,:,i);
    ...
    g_hat(1:na,i) = g_hat_model(1:na,1) - ob_ix(i-1)*H_atmos'*invR*(innov(:,i-1)-H_atmos*dz(1:na,i));
end
